...
with open('/home/ubuntu/flask/project/ChromeExtension/server/data/ProblemDict.json', 'r') as f:
    ProblemDict = json.load(f)
with open('/home/ubuntu/flask/project/ChromeExtension/server/data/ProblemTagsDict.json', 'r') as f:
    TagDict = json.load(f)
with open('/home/ubuntu/flask/project/ChromeExtension/server/data/level_to_tier.json', 'r') as f:
    TierDict = json.load(f)
  

def get_item2vec_problem(problem_id, submits, div):
    # 저장된 모델 불러오기
    model = Word2Vec.load("/home/ubuntu/flask/project/ChromeExtension/server/models/item2vec/word2vec_model.bin") 
...
def vae_recommend_problem(problem_list, origin_problem):
    model = tf.keras.models.load_model('/home/ubuntu/flask/project/ChromeExtension/server/models/VAE/VAE_model_ver2.h5', custom_objects={'vae_loss': vae_loss , 'vae' : vae, 'encoder' : encoder, 'decoder' : decoder})
